...
st.markdown('<div class="title-font">SALARY PREDICTOR</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle-font">for Fair Compensation</div>', unsafe_allow_html=True)

### FIRST DIVISION LINE ###
st.markdown("""<hr style="border: none; height: 4px; background-color: white; margin: 10px 0; box-shadow: none; opacity: 1;">""", unsafe_allow_html=True)
...
if st.session_state.selected_section != "Description" and st.session_state.selected_section != "H-1B Salaries Report":
    ...
    st.markdown("""<style>
        ...
        div[data-testid="stSelectbox"] > label {display: none; margin: 0; padding: 0;
        }

        /* Hide the label container of all date inputs */
        div[data-testid="stDateInput"] > label {display: none; margin: 0; padding: 0;
        }
    </style>""", unsafe_allow_html=True)
    ...
    st.markdown("""<style>
                    ...
                    div.stButton > button {color: white !important; background-color: #ff5757 !important; border: none !important; width: 300px; 
                                           height: 50px; font-size: 18px; border-radius: 8px; cursor: pointer;}

                    div.stButton > button:hover {background-color: #e04e4e !important; color: white !important;}
                </style>""", unsafe_allow_html=True)
    ...
    if clicked:
        ...
        if selected_value_company == "" or selected_value_job_title == "" or selected_value_location == "" or selected_value_branch == "":
            st.markdown("""<div style='margin-top: 30px; display: flex; justify-content: center;'>
